...
对论文“Anti-Windup in PID Control: Review, Analysis, and New Tuning Directions”进行了详细解读、补充与复现
...
作者主要针对对普通 PI 控制器进行仿真研究。**通过 AI** 针对 RM 常见情景，对使用不同策略的 PI、PID、PID+前馈 控制器通过 MATLAB 进行复现与补充。

**代码与结果见 simulation 文件夹**。
...
| [瞬时反计算](####算法二：瞬时反计算_PI)                      | IBC     |
| [条件积分](####算法三：条件积分_PI)                          | CI      |
| [条件反计算](####算法四：条件反计算_PI)                      | CBC1    |
| [增量裁剪反计算](####算法六：增量裁剪反计算_PI)              | CBC2    |
| [分段式动态反计算](####算法五：分段式动态反计算_PI)          | DBC_STR |
| [广义反计算](###广义反计算（Generalized Back-Calculation）)  | GBC     |

---

作者使用IAE（绝对误差积分）进行性能比较：

$$
IAE=\sum_{k=1}^{N}\lvert e(k) \rvert
$$
我在这里增加一种比较维度ITAE，增加了相应相应速度权重：
$$
ITAE=\sum_{k=1}^{N}k\cdot\lvert e(k) \rvert
$$
...
作者在仿真中多运用 $R_S$ 作为 x 轴，但没有具体介绍各参数的变化方法。

在这里具体规定：

- $u_f$ 为无饱和限制情况下的最大 $u_c$. 对每一轮仿真，先跑一轮无饱和得到 $u_f$ 以计算 $R_S$，再跑正常有饱和
- $u_{lim}$ 参考真实情景，设固定值
- $u_0$ 取0
...
所有 PID 参数通过 [λ方法](##λ方法) 整定。λ 的取值由 x 决定：
...
模型：
$$
G_1(s)=\frac{1}{0.03s+1}e^{-Ls}
$$


研究范围：
$$
\frac{L}{T}\in \{0.02, 0.1, 0.3, 0.5 \}
$$
...
**结论：对于大多情况，[经典反计算](####算法一：动态反计算_PI)和[分段式动态反计算](###分段式动态反计算)都是最佳方案**

## 情景二：真实速度环

模型：
$$
G_2(s)=\frac{1}{(0.02s+1)(0.05s+1)}e^{-Ls}\\
T_{eq}=0.05\\
\frac{L}{T_{eq}}\in \{0.02, 0.1, 0.3, 0.5 \}\\
x \in \{0.2,0.5,0.8\}\\
u_{lim}=\pm1
$$


**结论：[经典反计算](####算法一：动态反计算_PI)和[分段式动态反计算](###分段式动态反计算)表现稳定。在PI控制器的低 $R_S$ 场景，[条件积分](####算法三：条件积分_PI)和[增量裁剪反计算](####算法六：增量裁剪反计算_PI)展现出约10%优势；在PID控制器的低 $R_S$ 场景，[条件积分](####算法三：条件积分_PI)和[条件反计算](####算法四：条件反计算_PI)展现出略微优势；在PIDF场景，[增量裁剪反计算](####算法六：增量裁剪反计算_PI)炸机。**

## 情景三：平衡pitch环

模型：
$$
G_4(s)=\frac{1}{0.04s^2+0.1s+1}e^{-Ls}\\
T_{eq}=0.05\\
\frac{L}{T_{eq}}\in \{0.02, 0.1, 0.3, 0.5 \}\\
x \in \{0.2,0.5,0.8\}\\
u_{lim}=\pm1
$$
所有方案炸机，应当是λ方法的锅，之后有时间用 GWO 整定了系数再测试。
