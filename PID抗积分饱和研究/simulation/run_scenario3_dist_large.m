%% 情景三：平衡pitch环 - 抗一次性大幅扰动
% 作者：Luciano
% 日期：2026-06-09
% 描述：测试控制器抵抗一次性大幅扰动的能力
%       目标：保持输出在0
%       输出：3_2(PI), 3_4(PID), 3_6(PIDF)

function results = run_scenario3_dist_large()
    clc; clear; close all;
    ...
    %% 生成一次性大幅扰动（阶跃扰动）
    d = zeros(N, 1);
    d_start = round(5 / Ts);  % t=5s开始
    d_end = round(7 / Ts);    % t=7s结束
    d(d_start:d_end) = 0.8;   % 大幅扰动
    ...
    %% ========== 主循环 ==========
    fprintf('=== 情景三 抗一次性大幅扰动 ===\n');
    fprintf('扰动: 阶跃 0.8, t=5~7s\n');
    fprintf('==========================================\n');
        ...
        for j = 1:num_x
            ...
            for s = 1:num_pid
                ...
                [results.PIDF.IAE(i,j,s), results.PIDF.ITAE(i,j,s)] = ...
                    simulate_dist(K, a2, a1, a0, L, Kp_pid, Ki_pid, Kd_pid, Ts, N, w, d, u_lim, strategies_pid{s}, T_eff, 'PIDF', 0, 0);
            end
            fprintf('\n');
        end
    end
    ...
    save('scenario3_dist_large_results.mat', 'results');
    fprintf('完成！\n');
end
...
function plot_time_domain(results, K, a2, a1, a0, T_eq, Ts, N, w, d, u_lim, T_eff, folders)
    ...
    for s = 1:num_pi
        [~, ~, y] = simulate_dist_full(K, a2, a1, a0, L, Kp_pi, Ki_pi, 0, Ts, N, w, d, u_lim, strategies_pi{s}, T_eff, 'PI');
        plot(t(show_idx), y(show_idx), '-', 'Color', colors_pi(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pi{s});
    end
    xlabel('时间 (s)'); ylabel('输出 y'); title(sprintf('PI 输出 (L/T=%.2f, x=%.1f)', L_T, x));
    legend('Location','best','FontSize',7); grid on; hold off;

    subplot(2,1,2); hold on;
    plot(t(show_idx), d(show_idx), 'k-', 'LineWidth', 1.5, 'DisplayName', '扰动');
    for s = 1:num_pi
    ...
    plot(t(show_idx), w(show_idx), 'k--', 'LineWidth', 1.5, 'DisplayName', 'w');
    for s = 1:num_pid
        [~, ~, y] = simulate_dist_full(K, a2, a1, a0, L, Kp_pid, Ki_pid, Kd_pid, Ts, N, w, d, u_lim, strategies_pid{s}, T_eff, 'PID');
        plot(t(show_idx), y(show_idx), '-', 'Color', colors_pid(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pid{s});
    end
    ...
    plot(t(show_idx), w(show_idx), 'k--', 'LineWidth', 1.5, 'DisplayName', 'w');
    for s = 1:num_pid
        [~, ~, y] = simulate_dist_full(K, a2, a1, a0, L, Kp_pid, Ki_pid, Kd_pid, Ts, N, w, d, u_lim, strategies_pid{s}, T_eff, 'PIDF', 0, 0);
        plot(t(show_idx), y(show_idx), '-', 'Color', colors_pid(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pid{s});
    end
