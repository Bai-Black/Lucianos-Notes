...
function results = run_scenario3_PID()
    clc; clear; close all;
        ...
        for j = 1:num_x
            ...
            for w_idx = 1:num_w
                ...
                for s = 1:num_strategies
                    [IAE_val, ITAE_val, ~, ~, converged] = simulate_pid_osc(...
                        K, a2, a1, a0, L, Kp, Ki, Kd, Ts, N, w_signal, u_lim, strategies{s});
                    if converged
    ...
    save('scenario3_PID_results.mat', 'results');
    fprintf('完成！结果保存到 scenario3_PID_results.mat\n');
end

%% ========== 无饱和仿真 ==========
function [y, u_max] = simulate_without_saturation_osc(K, a2, a1, a0, L, Kp, Ki, Ts, N, w)
    y = zeros(N, 1); u_c = zeros(N, 1); e = zeros(N, 1); u_i = zeros(N, 1);
    ...
    for k = 2:N
        ...
        if k > delay_steps, u_delayed = u_c(k - delay_steps); else, u_delayed = 0; end
...
%% ========== PID仿真 ==========
function [IAE, ITAE, y, u_sat, converged] = simulate_pid_osc(K, a2, a1, a0, L, Kp, Ki, Kd, Ts, N, w, u_lim, strategy)
    y = zeros(N, 1); u_c = zeros(N, 1); u_sat = zeros(N, 1);
    e = zeros(N, 1); e_sat = zeros(N, 1); u_i = zeros(N, 1); u_d = zeros(N, 1);
    y1 = zeros(N, 1); y2 = zeros(N, 1);

    delay_steps = round(L / Ts);
    e_prev = 0; d_prev = 0; y_prev = 0; y_prev2 = 0; u_d_prev = 0;
    T_eff = a2/a1;  % 等效时间常数
...
function plot_normalized_results(results, output_folder)
    ...
    num_strategies = length(results.strategies); strategies = results.strategies;
    idx_dbc = find(strcmp(strategies, 'DBC_CLA')); colors = lines(num_strategies);

    for i = 1:num_L_T
        for j = 1:num_x
            R_S_vec = squeeze(results.R_S(i, j, :));

            figure('Visible', 'off'); hold on; has_data = false;
            for s = 1:num_strategies
                ...
                plot(R_S_vec(valid), IAE_vec(valid) ./ squeeze(results.IAE(i, j, valid, idx_dbc)), '-o', 'Color', colors(s,:), 'LineWidth', 2, 'MarkerSize', 4, 'DisplayName', strategies{s});
                has_data = true;
            end
            if has_data, xlabel('R_S'); ylabel('归一化 IAE'); title(sprintf('IAE归一化 (L/T=%.2f, x=%.1f)', results.L_T_values(i), results.x_values(j))); legend('Location', 'best'); grid on; xlim([0.1, 0.95]); ylim([0.5, 2.5]); end
            hold off;
            ...
            for s = 1:num_strategies
                ...
                if sum(valid) < 2, continue; end
                plot(R_S_vec(valid), ITAE_vec(valid) ./ squeeze(results.ITAE(i, j, valid, idx_dbc)), '-o', 'Color', colors(s,:), 'LineWidth', 2, 'MarkerSize', 4, 'DisplayName', strategies{s});
                has_data = true;
            end
            if has_data, xlabel('R_S'); ylabel('归一化 ITAE'); title(sprintf('ITAE归一化 (L/T=%.2f, x=%.1f)', results.L_T_values(i), results.x_values(j))); legend('Location', 'best'); grid on; xlim([0.1, 0.95]); ylim([0.5, 2.5]); end
            hold off;
...
function plot_time_domain(results, K, a2, a1, a0, T_eq, Ts, N, w_values, u_lim, output_folder)
    num_L_T = length(results.L_T_values); num_x = length(results.x_values);
    num_strategies = length(results.strategies); strategies = results.strategies;
    t = (0:N-1) * Ts; colors = lines(num_strategies);

    fprintf('绘制时域图...\n');
    for i = 1:num_L_T
        L_T = results.L_T_values(i); L = L_T * T_eq;
        if L_T <= 0.1, target_RS = 0.2; else, target_RS = 0.3; end

        for j = 1:num_x
            x = results.x_values(j); lambda = x * T_eq;
            R_S_vec = squeeze(results.R_S(i, j, :));
            ...
            w_signal = zeros(N, 1); w_signal(round(1/Ts):end) = w_val;
            Kp = T_eq / (K * (lambda + L)); Ti = T_eq; Ki = Kp / Ti;

            y_all = zeros(N, num_strategies); u_sat_all = zeros(N, num_strategies);
            for s = 1:num_strategies
                [~, ~, y_all(:, s), u_sat_all(:, s)] = simulate_pi_osc(K, a2, a1, a0, L, Kp, Ki, Ts, N, w_signal, u_lim, strategies{s});
            end

            show_idx = t <= 5;
            figure('Visible', 'off'); hold on;
            plot(t(show_idx), w_signal(show_idx), 'k--', 'LineWidth', 2, 'DisplayName', 'w');
            for s = 1:num_strategies, plot(t(show_idx), y_all(show_idx, s), '-', 'Color', colors(s,:), 'LineWidth', 1.5, 'DisplayName', strategies{s}); end
            xlabel('时间 (s)'); ylabel('输出 y'); title(sprintf('系统输出 (L/T=%.2f, x=%.1f, R_S≈%.1f)', L_T, x, target_RS));
            ...
            figure('Visible', 'off'); hold on;
            yline(u_lim, 'k--', 'LineWidth', 1.5, 'DisplayName', 'u_{lim}'); yline(-u_lim, 'k--', 'LineWidth', 1.5, 'HandleVisibility', 'off');
            for s = 1:num_strategies, plot(t(show_idx), u_sat_all(show_idx, s), '-', 'Color', colors(s,:), 'LineWidth', 1.5, 'DisplayName', strategies{s}); end
            xlabel('时间 (s)'); ylabel('u_{sat}'); title(sprintf('控制器输出 (L/T=%.2f, x=%.1f, R_S≈%.1f)', L_T, x, target_RS));
