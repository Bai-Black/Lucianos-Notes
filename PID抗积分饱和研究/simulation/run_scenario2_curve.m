...
% 作者：Luciano
% 日期：2026-06-09
% 描述：二阶系统曲线跟踪，比较PI/PID/PIDF控制器
%       输出：2_3(PI), 2_4(PID), 2_5(PIDF)

function results = run_scenario2_curve()
    clc; clear; close all;
    ...
    for i = 1:num_L_T
        ...
        for j = 1:num_x
            ...
            for s = 1:num_strategies_pi
                ...
                [IAE_val, ITAE_val, y_pi_all(:, s), u_sat_pi_all(:, s)] = simulate_pi_2nd(...
                    K, T1, T2, L, Kp_pi, Ki_pi, Ts, N, w, u_lim, strategies_pi{s});
                results.PI.IAE(i, j, s) = IAE_val;
                results.PI.ITAE(i, j, s) = ITAE_val;
            end
            fprintf('\n');
            ...
            for s = 1:num_strategies_pid
                ...
                [IAE_val, ITAE_val, y_pid_all(:, s), u_sat_pid_all(:, s)] = simulate_pid_2nd(...
                    K, T1, T2, L, Kp_pid, Ki_pid, Kd_pid, Ts, N, w, u_lim, strategies_pid{s});
                results.PID.IAE(i, j, s) = IAE_val;
                results.PID.ITAE(i, j, s) = ITAE_val;
            end
            fprintf('\n');
            ...
            for s = 1:num_strategies_pid
                fprintf('%s ', strategies_pid{s});
                [IAE_val, ITAE_val, y_pidf_all(:, s), u_sat_pidf_all(:, s)] = simulate_pidf_2nd(...
                    K, T1, T2, L, Kp_pid, Ki_pid, Kd_pid, Kff, Kff_d, Ts, N, w, u_lim, strategies_pid{s});
                results.PIDF.IAE(i, j, s) = IAE_val;
                results.PIDF.ITAE(i, j, s) = ITAE_val;
            end
            fprintf('\n');
    ...
    save('scenario2_curve_results.mat', 'results');
    fprintf('完成！结果保存到 scenario2_curve_results.mat\n');
end
...
function w = generate_smooth_curve(t, N)
    w_raw = zeros(N, 1);
    for k = 1:N
        w_raw(k) = 0.5 * sin(0.5*t(k)) + 0.3 * sin(1.2*t(k) + 0.5) ...
                 + 0.15 * sin(2.5*t(k) + 1.0) + 0.1 * cos(0.3*t(k));
    end
    w = w_raw - min(w_raw);
    w = w / max(w) * 0.7 + 0.1;
end
...
%% ========== PI仿真（二阶系统） ==========
function [IAE, ITAE, y, u_sat] = simulate_pi_2nd(K, T1, T2, L, Kp, Ki, Ts, N, w, u_lim, strategy)
    ...
    for k = 2:N
        e(k) = w(k) - y(k-1);
        params.e = e(k); params.e_prev = e(max(k-1,1));
        params.e_sat_prev = e_sat(k-1); params.Kp = Kp; params.Ki = Ki;
        params.Ts = Ts; params.u_i_prev = u_i(k-1);
        ...
        params.w = w(k); params.K = K; params.T = T2; params.L = L;
...
%% ========== PID仿真（二阶系统） ==========
function [IAE, ITAE, y, u_sat] = simulate_pid_2nd(K, T1, T2, L, Kp, Ki, Kd, Ts, N, w, u_lim, strategy)
    y = zeros(N, 1); u_c = zeros(N, 1); u_sat = zeros(N, 1);
    ...
    for k = 2:N
        ...
        params.y = y(max(k-1,1)); params.y_prev = y_prev; params.y_prev2 = y_prev2;
        params.w = w(k); params.K = K; params.T = T2; params.L = L; params.u_d_prev = u_d_prev;
...
%% ========== PIDF仿真（二阶系统） ==========
function [IAE, ITAE, y, u_sat] = simulate_pidf_2nd(K, T1, T2, L, Kp, Ki, Kd, Kff, Kff_d, Ts, N, w, u_lim, strategy)
    y = zeros(N, 1); u_c = zeros(N, 1); u_sat = zeros(N, 1);
    e = zeros(N, 1); e_sat = zeros(N, 1); u_i = zeros(N, 1); u_d = zeros(N, 1);
    y1 = zeros(N, 1); y2 = zeros(N, 1);
...
function plot_time_domain_curves(t, w, L_T, x, ...
    ...
    y_pidf, u_sat_pidf, u_lim, folders)
    ...
    for s = 1:num_pi
        plot(t(show_idx), y_pi(show_idx, s), '-', 'Color', colors_pi(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pi{s});
    end
    ...
    for s = 1:num_pi
        plot(t(show_idx), u_sat_pi(show_idx, s), '-', 'Color', colors_pi(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pi{s});
    end
    ...
    plot(t(show_idx), w(show_idx), 'k--', 'LineWidth', 2, 'DisplayName', '设定值');
    for s = 1:num_pid
        plot(t(show_idx), y_pid(show_idx, s), '-', 'Color', colors_pid(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pid{s});
    end
    ...
    yline(u_lim, 'k--', 'LineWidth', 1, 'DisplayName', 'u_{lim}'); yline(-u_lim, 'k--', 'LineWidth', 1, 'HandleVisibility', 'off');
    for s = 1:num_pid
        plot(t(show_idx), u_sat_pid(show_idx, s), '-', 'Color', colors_pid(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pid{s});
    end
    ...
    plot(t(show_idx), w(show_idx), 'k--', 'LineWidth', 2, 'DisplayName', '设定值');
    for s = 1:num_pid
        plot(t(show_idx), y_pidf(show_idx, s), '-', 'Color', colors_pid(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pid{s});
    end
    ...
    yline(u_lim, 'k--', 'LineWidth', 1, 'DisplayName', 'u_{lim}'); yline(-u_lim, 'k--', 'LineWidth', 1, 'HandleVisibility', 'off');
    for s = 1:num_pid
        plot(t(show_idx), u_sat_pidf(show_idx, s), '-', 'Color', colors_pid(s,:), 'LineWidth', 1.5, 'DisplayName', strategies_pid{s});
    end
...
function plot_summary(results, folders)
    ...
    for i = 1:num_L_T
        for j = 1:num_x
            % PI
            figure('Visible', 'off');
            bar(squeeze(results.PI.IAE(i, j, :)));
            ...
            figure('Visible', 'off');
            bar(squeeze(results.PI.ITAE(i, j, :)));
            set(gca, 'XTickLabel', strategies_pi, 'XTickLabelRotation', 45);
            ...
            % PID
            figure('Visible', 'off');
            bar(squeeze(results.PID.IAE(i, j, :)));
            ...
            figure('Visible', 'off');
            bar(squeeze(results.PID.ITAE(i, j, :)));
            set(gca, 'XTickLabel', strategies_pid, 'XTickLabelRotation', 45);
            ...
            % PIDF
            figure('Visible', 'off');
            bar(squeeze(results.PIDF.IAE(i, j, :)));
            set(gca, 'XTickLabel', strategies_pid, 'XTickLabelRotation', 45);
            ...
            figure('Visible', 'off');
            bar(squeeze(results.PIDF.ITAE(i, j, :)));
            set(gca, 'XTickLabel', strategies_pid, 'XTickLabelRotation', 45);
