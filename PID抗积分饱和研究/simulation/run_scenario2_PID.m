...
function results = run_scenario2_PID()
    clc; clear; close all;
        ...
        for j = 1:num_x
            ...
            for w_idx = 1:num_w
                ...
                for s = 1:num_strategies
                    [IAE_val, ITAE_val, ~, ~, converged] = simulate_pid_2nd(...
                        K, T1, T2, L, Kp, Ki, Kd, Ts, N, w_signal, u_lim, strategies{s});
                    if converged
    ...
    save('scenario2_PID_results.mat', 'results');
    fprintf('完成！结果保存到 scenario2_PID_results.mat\n');
end
...
%% ========== PID仿真（二阶系统） ==========
function [IAE, ITAE, y, u_sat, converged] = simulate_pid_2nd(K, T1, T2, L, Kp, Ki, Kd, Ts, N, w, u_lim, strategy)
    y = zeros(N, 1);
    u_c = zeros(N, 1);
    u_sat = zeros(N, 1);
    e = zeros(N, 1);
...
function plot_time_domain(results, K, T1, T2, T_eq, Ts, N, w_values, u_lim, output_folder)
    ...
    for i = 1:num_L_T
        ...
        for j = 1:num_x
            ...
            for s = 1:num_strategies
                [~, ~, y_all(:, s), u_sat_all(:, s)] = simulate_pid_2nd(...
                    K, T1, T2, L, Kp, Ki, Kd, Ts, N, w_signal, u_lim, strategies{s});
            end
